...
void Tool::calibrateTray(cv::Mat image){
	...
	while (keyPress(cvWaitKey(33))){		
		...
		if (rotatedRects.size() > 0){
			cv::RotatedRect tray = detectTray(rotatedRects);
...
void Tool::calibrateStock(cv::Mat image){
	...
	imgThresholded = thresholdImage(
		imgHSV, 
		stockLowHue, stockHighHue, 
		stockLowSat, stockHighSat, 
		stockLowVal, stockHighVal);
	cv::findContours(imgThresholded, contours, CV_RETR_LIST, CV_CHAIN_APPROX_SIMPLE, cv::Point(0, 0));
	cv::approxPolyDP(cv::Mat(contours[0]), contours[0], 3, true);
	...
	while (keyPress(cvWaitKey(33))){			
		...
		cv::rectangle(imgLines, rect, cv::Scalar(255, 255, 0), 2);	
...
void Tool::writeFile(std::string profile){
	std::ofstream outputFile("save/" + profile + ".sav", std::ios::binary);
	outputFile.write((char*)this, sizeof(Tool));
	outputFile.close();
	std::cout << ">> Tool Saved: " << profile << "\n";
}

void Tool::readFile(std::string profile){
	std::ifstream  inputFile("save/" + profile + ".sav", std::ios::binary);
	inputFile.read((char*)this, sizeof(Tool));
	inputFile.close();
	std::cout << ">> Tool Loaded: " << profile << "\n";
...
cv::RotatedRect Tool::detectTray(cv::vector<cv::RotatedRect> rotatedRects){
	for (int i = 0; i < rotatedRects.size(); i++)
		if (validTray(rotatedRects[i]))
			return rotatedRects[i];
	return rotatedRects[0];
}

bool Tool::validTray(cv::RotatedRect tray){
	return length(tray.size.width, TRAYC) <= TRAYWIDTH + TRAYWIDTHRANGE &&
		length(tray.size.width, TRAYC) >= TRAYWIDTH - TRAYWIDTHRANGE &&
		length(tray.size.height, TRAYC) <= TRAYHEIGHT + TRAYHEIGHTRANGE &&
		length(tray.size.height, TRAYC) >= TRAYHEIGHT - TRAYHEIGHTRANGE;
}
...
cv::vector<Tablet> Tool::detectCircles(cv::Mat image, cv::Mat imgLines, cv::Rect validArea, CameraType type){
	...
	cv::HoughCircles(image, circles, CV_HOUGH_GRADIENT, 1, image.rows/8, canny, accumulator, 
		cvRound(((TABLETRADIUS - 4)/10) * measure), cvRound(((TABLETRADIUS + 4)/10) * measure));
	...
	{		
		...
		if (validArea.contains(center)) {
			tablets.push_back(Tablet(circles[i]));
...
cv::vector<Tablet> Tool::detectCircles(cv::Mat image, cv::Rect validArea, CameraType type){
	...
	/// Apply the Hough Transform to find the circles
	cv::HoughCircles(image, circles, CV_HOUGH_GRADIENT, 1, image.rows/8, canny, accumulator, 
		cvRound(((TABLETRADIUS - 3)/10) * measure), cvRound(((TABLETRADIUS + 3)/10) * measure));
	...
	{		
		...
		if (validArea.contains(center)) {
			...
			int x, y, d;			
			tablet.center.x = cvRound(circles[i][0]);
			tablet.center.y = cvRound(circles[i][1]);
			offset(tablet.center, stockPoint, &x, &y, &d);
			tablet.radius = length(cvRound(circles[i][2]), STOCKC);
			tablet.offsetX = length(x, STOCKC);
			tablet.offsetY = length(y, STOCKC);
...
void Tool::detectTablet(Tablet* tablet, cv::Mat image, CameraType type){
	...
	imgThresholded = thresholdImage(imgHSV, stockBlueLowHue, stockBlueHighHue, stockBlueLowSat, stockBlueHighSat, stockBlueLowVal, stockBlueHighVal);
	point = getPointofObject(imgThresholded);
	if (isDetectedTablet(tablet, point, BLUE))
		return;

	imgThresholded = thresholdImage(imgHSV, stockGreenLowHue, stockGreenHighHue, stockGreenLowSat, stockGreenHighSat, stockGreenLowVal, stockGreenHighVal);
	point = getPointofObject(imgThresholded);
	if (isDetectedTablet(tablet, point, GREEN))
		return;

	imgThresholded = thresholdImage(imgHSV, stockRedLowHue, stockRedHighHue, stockRedLowSat, stockRedHighSat, stockRedLowVal, stockRedHighVal);
	point = getPointofObject(imgThresholded);
	if (isDetectedTablet(tablet, point, RED))
		return;

	imgThresholded = thresholdImage(imgHSV, stockBlackLowHue, stockBlackHighHue, stockBlackLowSat, stockBlackHighSat, stockBlackLowVal, stockBlackHighVal);
	point = getPointofObject(imgThresholded);
	if (isDetectedTablet(tablet, point, BLACK))
		return;

	imgThresholded = thresholdImage(imgHSV, stockWhiteLowHue, stockWhiteHighHue, stockWhiteLowSat, stockWhiteHighSat, stockWhiteLowVal, stockWhiteHighVal);
	point = getPointofObject(imgThresholded);
...
bool Tool::isDetectedTablet(Tablet* tablet, cv::Point mask, Colour colour){
	...
	if (length(d, STOCKC) > 2 ||
		length(d, STOCKC) < -2) // Allowed distance from masked point
		return false;
...
std::string Tool::tabletToString(Tablet tablet, CameraType type){
	...
	tablet.valid ?
		...
		tablet.center.x, tablet.center.y, length(x, type) - TABLETXOFFSET, 
		length(y, type) + TABLETYOFFSET, length(tablet.radius, type)) :
...
// Tablet Class : Public
Tablet::Tablet(){
	Tablet(cv::Vec3f(0, 0, 0));
}

Tablet::Tablet(cv::Vec3f circle){
...
Tray::Tray(cv::RotatedRect tray, Tool tool){
	int x, y, d;
	tool.offset(tray.center, tool.trayPoint, &x, &y, &d);

	width = tool.length(tray.size.width, TRAYC);
	height = tool.length(tray.size.height, TRAYC);
	angle = tray.angle;;
	offsetX = tool.length(x, TRAYC);
	offsetY = tool.length(y, TRAYC);
